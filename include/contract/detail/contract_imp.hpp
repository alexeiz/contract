...
#define contract_fun__                                                                                                 \
    auto contract_obj__ =                                                                                              \
        contract::detail::contractor<void *>(0) + [&](contract::detail::contract_context const & contract_context__)

// Define contract for a method.
#define contract_this__                                                                                                \
    auto contract_obj__ = contract::detail::contractor<std::remove_reference<decltype(*this)>::type>(this) +           \
                          [&](contract::detail::contract_context const & contract_context__)

// Define contract for a constructor.
#define contract_ctor__                                                                                                \
    auto contract_obj__ =                                                                                              \
        contract::detail::contractor<std::remove_reference<decltype(*this)>::type>(this, false, true) +                \
        [&](contract::detail::contract_context const & contract_context__)

// Define contract for a destructor.
#define contract_dtor__                                                                                                \
    auto contract_obj__ =                                                                                              \
        contract::detail::contractor<std::remove_reference<decltype(*this)>::type>(this, true, false) +                \
        [&](contract::detail::contract_context const & contract_context__)
...
#define contract_class__                                                                                               \
    ...
    {                                                                                                                  \
        return contract_context__;                                                                                     \
                                                                                                                       ...
                                                                                                                       \
    void class_contract__(contract::detail::contract_context const & contract_context__) const
...
#define contract_loop__ if (contract::detail::contract_context contract_context__{false, false, true})
    ...
    {                                                                                                                  \
        if (contract_context__.check_##TYPE() && !(COND))                                                              \
            contract::handle_violation(                                                                                \
                contract::violation_context(contract::type::TYPE, MSG, #COND, __FILE__, __LINE__));                    \
    } while (0)
...
struct fun_contract
{
    explicit fun_contract(ContrFunc f, bool enter = true, bool exit = true)
        : contr_(f)
        , exit_{exit}
    {
    ...
    ~fun_contract() noexcept(false) { contr_(contract_context{false, true, exit_}); }
...
struct has_class_contract
{
    ...
    static auto test(int)
        -> decltype(std::declval<U>().class_contract__(std::declval<contract_context>()), std::true_type{});
    template <typename U>
...
struct base_class_contract
{
    ...
    static void enforce(Derived * obj, contract_context const & context)
    {}
};
...
{
    template <typename T>
    static void do_enforce(T * obj, contract_context const & context,
...
{
    explicit contractor(T const *, bool = true, bool = true) {}
...
{
    explicit contractor(T const * obj, bool enter = true, bool exit = true)
        : obj_{obj}
        , enter_{enter}
        , exit_{exit}
    {}
...
// execution.
inline void default_handler(violation_context const & context)
{
...
{
    static violation_handler current_handler;
};
...
inline void handle_violation(violation_context const & context)
{
...
inline violation_handler set_handler(violation_handler new_handler)
{
...
inline violation_handler get_handler() { return detail::handler_holder<>::current_handler; }
